function addBanner() {
    const banner = toDOM(`
      <div class="focusmode-banner">
        focus-mode is active. Click to disable.
      </div>
    `)
    ...
    banner.addEventListener("click", () => {
      ...
      const dialog = toDOM(`
        <div class="focusmode-dialog">
          <div>
            Breathe<br/>
            <div id="breath-circle"></div>
            <br/>
            What do you need right now?<br/>
            <input type="text" id="focusmode-why" autocomplete="off"><br/>
            <br/>
            <div id="focusmode-message"></div>
          </div>
        </div>
      `)
      document.body.appendChild(dialog)
      const why = document.getElementById("focusmode-why")
      const message = document.getElementById("focusmode-message")
      ...
      why.addEventListener("input", () => {
        if (why.value.length > 5) {
          message.innerHTML = `To unblock, click and hold for <span class='focusmode-countdown'>${delayInSeconds}</span> seconds.`
        } else {
          message.innerHTML = ``
        }
      })
      ...
      dialog.addEventListener("mousedown", () => {
        ...
        interval = setInterval(async () => {
          ...
          if (remaining < 0) {
            ...
            document.querySelector(".focusmode-countdown").textContent = remaining.toFixed(0)
          }
        }, 1000)
      })
  ...
  const disableClassName = "disable-focusmode"
